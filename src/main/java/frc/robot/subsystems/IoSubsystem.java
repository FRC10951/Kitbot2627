...
public class IoSubsystem extends SubsystemBase {
  ...
  /** Absolute stator current (A) from the intake Talon FX, refreshed in periodic. */
  private double cachedIntakeStatorAmps;
  ...
  private IntakeJamPhase intakeJamPhase = IntakeJamPhase.NORMAL;
  private double intakeJamPhaseStartSeconds;
  private double jamConditionStreakSeconds;
  /** {@link Timer#getFPGATimestamp()} when inward intaking last began (for grace). */
  private double intakeInwardStartedAtSeconds;
  private double jamLastPeriodicSeconds = -1.0;
  private double intakeJamIgnoreUntilSeconds;
  ...
  public void periodic() {
    ...
    cachedIntakeStatorAmps =
        RobotBase.isSimulation()
            ? 0.0
            : Math.abs(intakeMotor.getStatorCurrent().getValue().in(Amps));

    double now = Timer.getFPGATimestamp();
    double dt =
        jamLastPeriodicSeconds < 0.0
            ? TimedRobot.kDefaultPeriod
            : Math.max(1e-6, now - jamLastPeriodicSeconds);
    jamLastPeriodicSeconds = now;

    updateShooterControl();
    if (INTAKE_JAM_REVERSE_FAILSAFE_ENABLED) {
      if (advanceIntakeJamRecovery(now)) {
        loaderMotor.set(0.0);
      } else if (tickIntakeJamDetection(now, dt)) {
        loaderMotor.set(0.0);
      } else {
        updateIntakeControl();
      }
    } else {
      if (intakeJamPhase != IntakeJamPhase.NORMAL) {
        resetIntakeJamState();
    ...
    SmartDashboard.putNumber("IO/Intake stator A", cachedIntakeStatorAmps);
    SmartDashboard.putBoolean("IO/Intake jam failsafe enabled", INTAKE_JAM_REVERSE_FAILSAFE_ENABLED);
    SmartDashboard.putString("IO/Intake jam phase", intakeJamPhase.name());
    SmartDashboard.putNumber(
  ...
  public void enableIntakeAtSpeed(double targetSpeedRpm) {
    boolean wasInward = intakeEnabled && intakeTargetSpeedRpm < 0.0;
    boolean nowInward = targetSpeedRpm < 0.0;
    intakeEnabled = true;
    intakeTargetSpeedRpm = targetSpeedRpm;
    if (nowInward && !wasInward) {
      intakeInwardStartedAtSeconds = Timer.getFPGATimestamp();
      jamConditionStreakSeconds = 0.0;
    }
    if (!nowInward) {
      jamConditionStreakSeconds = 0.0;
      if (intakeJamPhase != IntakeJamPhase.NORMAL) {
        intakeJamPhase = IntakeJamPhase.NORMAL;
        intakeVelocityPid.reset();
      }
    }
  }
  ...
  private void resetIntakeJamState() {
    intakeJamPhase = IntakeJamPhase.NORMAL;
    jamConditionStreakSeconds = 0.0;
    intakeJamIgnoreUntilSeconds = 0.0;
  }

  /**
   * When intaking inward, detect high stator or near-zero speed (with minimum
   * current) after a grace period; debounce, then begin pause + reverse recovery.
   *
   * @return true if this tick entered jam recovery (caller must not run closed-loop
   *     intake).
   */
  private boolean tickIntakeJamDetection(double now, double dt) {
    if (RobotBase.isSimulation()
        || !intakeEnabled
        || intakeTargetSpeedRpm >= 0.0) {
      return false;
    }
    if (now < intakeJamIgnoreUntilSeconds) {
      jamConditionStreakSeconds = 0.0;
      return false;
    }

    boolean pastGrace = (now - intakeInwardStartedAtSeconds) >= INTAKE_JAM_GRACE_SECONDS;
    double stator = cachedIntakeStatorAmps;
    double rpmMag = Math.abs(cachedIntakeRpm);
    boolean spike = stator >= INTAKE_JAM_STATOR_SPIKE_AMPS;
    boolean stall =
        rpmMag <= INTAKE_JAM_STALL_RPM && stator >= INTAKE_JAM_STALL_MIN_STATOR_AMPS;
    boolean jamNow = pastGrace && (spike || stall);

    if (jamNow) {
      jamConditionStreakSeconds += dt;
      if (jamConditionStreakSeconds >= INTAKE_JAM_DEBOUNCE_SECONDS) {
        intakeJamPhase = IntakeJamPhase.PAUSING;
        intakeJamPhaseStartSeconds = now;
        intakeVelocityPid.reset();
        intakeMotor.setVoltage(0.0);
        jamConditionStreakSeconds = 0.0;
        return true;
  ...
  /**
   * Drives pause-then-reverse jam clearing while {@link IntakeJamPhase} is not
   * {@link IntakeJamPhase#NORMAL}.
   *
   * @return true if recovery still owns the intake motor this tick.
   */
  private boolean advanceIntakeJamRecovery(double now) {
    switch (intakeJamPhase) {
      case NORMAL:
        return false;
      case PAUSING:
        intakeMotor.setVoltage(0.0);
        intakeVelocityPid.reset();
        if (now - intakeJamPhaseStartSeconds >= INTAKE_JAM_PAUSE_SECONDS) {
          intakeJamPhase = IntakeJamPhase.REVERSING;
          intakeJamPhaseStartSeconds = now;
        }
        return true;
      case REVERSING:
        intakeMotor.setVoltage(
            Math.min(INTAKE_MAX_VOLTAGE, INTAKE_JAM_REVERSE_VOLTAGE));
        if (now - intakeJamPhaseStartSeconds >= INTAKE_JAM_REVERSE_SECONDS) {
          intakeJamPhase = IntakeJamPhase.NORMAL;
          intakeVelocityPid.reset();
          intakeJamIgnoreUntilSeconds = now + INTAKE_JAM_POST_RECOVERY_COOLDOWN_SECONDS;
          jamConditionStreakSeconds = 0.0;
          return false;
        }
        return true;
