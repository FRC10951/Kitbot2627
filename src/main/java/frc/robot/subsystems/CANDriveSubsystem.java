...
public class CANDriveSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    ...
    SmartDashboard.putBoolean("Drive/Large shake while shooting", INTAKE_WIGGLE_LARGE_SHAKE_ENABLED);
  ...
  private Command commandIntakeWiggleOneCycle(double wiggleSpeed, double halfPeriodSeconds) {
    Command first =
        this.run(() -> driveArcade(wiggleSpeed * INTAKE_WIGGLE_FORWARD_SPEED_SCALE, 0))
            .withTimeout(halfPeriodSeconds);
  ...
  public Command commandIntakeWiggleWhileShooting() {
    ...
    if (!INTAKE_WIGGLE_LARGE_SHAKE_ENABLED) {
      return commandIntakeWiggle(INTAKE_WIGGLE_SPEED, half)
          .repeatedly()
          .finallyDo(interrupted -> stop());
    }
